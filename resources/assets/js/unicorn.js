class Unicorn {
  ...
  static initNewItem(count, elementContainer, modelType, itemOptions) {
    let item =
      ...
      '_item_' +
      count +
      '" class="card-panel"><div class="row"><div class="input-field col s12 l8"> <select id="item_name_' +
      count +
      '" name="item_name[]" class="item-list-selector" data-parsley-required="true" data-parsley-trigger="change"><option disabled="" selected="selected" value="">Pick an Item or Create a new one</option> </select> <label for="item_name_' +
      count +
      '" class="label-validation">Name</label> <span class="helper-text"></span></div><div class="input-field col s6 l2"> <input id="item_quantity_' +
      count +
      '" name="item_quantity[]" class="item-quantity-input" type="number" data-parsley-required="true" data-parsley-trigger="change" data-parsley-min="1" placeholder="Item Quantity"> <label for="item_quantity_' +
      count +
      '" class="label-validation">Quantity</label> <span class="helper-text"></span></div><div class="input-field col s6 l2"> <input id="item_price_' +
      count +
      '" name="item_price[]" class="item-price-input" type="number" step="0.01" data-parsley-required="true" data-parsley-trigger="change" placeholder="Item Price"> <label for="item_price_' +
      count +
      '" class="label-validation">Price</label> <span class="helper-text"></span></div><div class="input-field col s12 mtop30"><textarea id="item_description_' +
      count +
      '" name="item_description[]" class="item-description-textarea trumbowyg-textarea" data-parsley-required="false" data-parsley-trigger="change" placeholder="Item Description"></textarea><label for="item_description_' +
      count +
      '" class="label-validation">Description</label> <span class="helper-text"></span></div></div><div class="row"> <button data-id="false" data-count="' +
      count +
      '" class="' +
      modelType +
      '-item-delete-btn btn waves-effect waves-light col s12 m3 offset-m9 red">Delete</button></div></div>';
  ...
  static initItemConfirmationTrigger(
    parent_selector,
    selector,
    modal_selector,
    buttonSelector,
  ...
  static initConfirmationTrigger(
    parent_selector,
    selector,
    fqdn,
    model,
    ...
    this.initListener(parent_selector, selector, trigger, function (event, element) {
      ...
      $(modal_form_selector).attr(
        'action',
        '/' + fqdn + '/' + model + '/' + dataid + '/' + model_action
      );
      $(modal_selector).modal('open');
    });
  }
    ...
    $(selector)
      .parsley({
        ...
        errorsContainer: function (velem) {
          ...
          $errelem.attr(
            'data-error',
            window.Parsley.getErrorMessage(velem.validationResult[0].assert)
          );
          return true;
        },
        errorsWrapper: '',
      ...
      .on('field:error', function (velem) {
        if (velem.$element.is(':radio')) {
          ...
          velem.$element
            .parentsUntil('.row')
            .find('span.helper-text')
            .attr('data-error', window.Parsley.getErrorMessage(velem.validationResult[0].assert));
        } else if (velem.$element.is('select')) {
  ...
  static retrieveItemTemplate(baseEndpoint, itemTemplateId, element, callback) {
    if (
      typeof itemTemplateId !== typeof undefined &&
      itemTemplateId !== false &&
    ...
    $('select.item-list-selector').selectize({
      ...
      render: {
        option: function (data) {
          return (
            '<div class="option" data-selectable="" data-id="' +
            data.id +
            '" data-value="' +
            data.name +
            '">' +
