    ...
    <div class="mini-container">
        <div id="top-action-container" class="row desktop-only">
            <div class="col s12 mtop30 right">
                <form method="post" action="{{ route('invoice.send', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}" class="null-form">
                    {{ csrf_field() }}
                    <button class="btn btn-link waves-effect waves-dark null-btn" type="submit">Send Notification</button>
                </form>
                @can('create', \App\Models\Payment::class)
                <a class="btn btn-link waves-effect waves-dark" href="{{ route('payment.create', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}">
                    Log Payment
                </a>
                @endcan
                ...
                <a class="btn btn-link waves-effect waves-dark" href="{{ route('invoice.download', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}">
                    Save PDF
                </a>
                <a class="btn btn-link waves-effect waves-dark" href="{{ route('invoice.printview', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}">
                    Print
                </a>
            </div>
        </div>
        ...
        <div id="invoice-action-container" class="row mbtm0 desktop-only">
            <div class="col s12 right">
                @can('update', $invoice)
                <form method="post" action="{{ route('invoice.convert', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}" class="null-form">
                    {{ csrf_field() }}
                    <button class="btn btn-link waves-effect waves-dark null-btn" type="submit">Convert back to Quote</button>
                </form>
                @endcan
                @can('update', $invoice)
                <form method="post" action="{{ route('invoice.duplicate', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}" class="null-form">
                    {{ csrf_field() }}
                    <button class="btn blue darken-3 waves-effect waves-dark null-btn" type="submit">Clone</button>
                </form>
                @endcan
                @can('update', $invoice)
                <form method="post" action="{{ route('invoice.writeoff', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}" class="null-form">
                    {{ method_field('PATCH') }}
                    {{ csrf_field() }}
                ...
                @endcan
                @can('update', $invoice)
                <form method="post" action="{{ route('invoice.archive', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}" class="null-form">
                    {{ method_field('PATCH') }}
                    {{ csrf_field() }}
                ...
                @can('update', $invoice)
                @if(!$invoice->isLocked())
                <a href="{{ route('invoice.edit', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}" class="btn light-blue waves-effect waves-dark">
                    Edit
                </a>
                @endif
                @endcan
                ...
                <div id="change-history-container" class="change-history-container">
                    <div class="single-history-wrapper">
                        <div class="card single-history">
                            ...
                            @can('update', $invoice)
                            @if(!$invoice->isLocked())<a href="{{ route('invoice.edit', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}" class="btn btn-theme full-width mtop20">Edit</a>@endif
                            @else
                            <a href="{{ route('invoice.show', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ] ) }}" class="btn btn-theme full-width mtop20">View</a>
                            @endcan
                        </div>
                    </div>
        ...
        $(function() {
            ...
            function getSharedLink()
            {
                $.ajax({
                    ...
                    method: "PATCH",
                    url: "{{ route('invoice.share', [ 'invoice' => $invoice->id, 'company' => \App\Library\Poowf\Unicorn::getCompanyKey() ]) }}",
                })
                .done(function(data) {
