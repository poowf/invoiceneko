                            ...
                            @for($i = 0; $i < $permissions->count(); $i++)
                                ...
                                <div class="input-field col s6 m3">
                                    ...
                                    <div class="switch mtop20">
                                        <label>
                                            <input id="permissions-{{ $permissions[$i]->name }}" name="permissions[]" data-permission-type="{{ \Illuminate\Support\Str::slug(strtolower($permissions[$i]->type)) }}" type="checkbox" value="{{ $permissions[$i]->name }}">
                                            <span class="lever"></span>
                                            Allow
                                        </label>
                                    </div>
...
@section("scripts")
    <script type="text/javascript">
        "use strict";
        $(function() {
            $('#create-role').on('click', '.all-permission-selector', function (event) {
            ...
            $('#create-role').on('click', 'input[id^=permissions-update]', function (event) {
                let type = $(this).attr('data-permission-type');
            ...
            $('#create-role').on('click', 'input[id^=permissions-create]', function (event) {
                let type = $(this).attr('data-permission-type');
                if(!$('input[id=permissions-view-' + type +']').prop('checked'))
                {
                    $('input[id=permissions-view-' + type +']').prop('checked', true);
            ...
            $('#create-role').on('click', 'input[id^=permissions-delete]', function (event) {
                let type = $(this).attr('data-permission-type');
                if(!$('input[id=permissions-view-' + type +']').prop('checked'))
                {
                    $('input[id=permissions-view-' + type +']').prop('checked', true);
